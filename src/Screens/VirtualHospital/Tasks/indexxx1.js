        ...
        axios
            ...
            .then((response) => {
                ...
                for (let i = 0; i < this.state.allPatData.length; i++) {
                    ...
                    patientArray.push({
                        last_name: this.state.allPatData[i].last_name,
                        user_id: this.state.allPatData[i]._id,
                        alies_id: this.state.allPatData[i].alies_id,
                        image: this.state.allPatData[i].image,
                        first_name: this.state.allPatData[i].first_name,
                        profile_id: this.state.allPatData[i].profile_id,
                        type: this.state.allPatData[i].type,
                    })
                    // PatientList.push({ value: this.state.allPatData[i]._id, label: name })
                     PatientList1.push({ profile_id: this.state.allPatData[i].profile_id, value: this.state.allPatData[i]._id, name: name })
                }
        ...
        const userList = this.state.filteredUsers && this.state.filteredUsers.map(user => {
            return (
                <li key={user.id} style={{ background: this.myColor(user.id), color: this.color(user.id) }} value={user.profile_id}
                    onClick={() => { this.setState({ q: user.name, selectedUser: user._id }); this.toggle(user.id); this.setState({ filteredUsers: [] }) }}
