...
class Index extends Component {
  ...
  componentDidMount() {
    if (this.props.history.location?.state?.needUpload) {
      this.setState({
        needUpload: this.props.history.location?.state?.needUpload,
      });
    }
        ...
        .then((responce1) => {
          if (responce1.data.hassuccessed) {
            ...
            if (!this.state.needUpload) {
              if (this.state.patinfo?.email) {
                senddata.email = this.state.patinfo?.email;
              }
              if (this.state.patinfo?.mobile) {
                      ...
                      <Grid className="headercont headercontSec">
                        {this.state.needUpload && (
                          <Grid className="profileInfoSection">
                            <h1>
                              Upload Approval document of patient to the
                              hospital
                            </h1>
                        ...
                        <Grid className="profileInfoSection">
                          <h1>
                            Select Step of Patient flow and Add case number of
                            Patient
                          </h1>
                          <Grid item xs={12} md={8}>
                            <Grid className="headerCountTxt">
