...
class Index extends Component {
  ...
  handleOpenQues = () => {
    this.setState({
      openQues: true,
      openOpti: this.state.myQuestions?.type === 'classic' ? true : false,
    });
  };
  ...
  deleteQuestions = (index) => {
    var QuesAy =
      this.state.myQuestions[0]?.length > 0 &&
  ...
  handleSubmit = () => {
    ...
    let { Plz_select_question_type, Title_cant_be_empty, Ques_cant_be_empty } =
      translate;
    ...
    if (
      !myQuestions[length - 1]?.type ||
      myQuestions[length - 1]?.type.length < 1
    ) {
      this.setState({ errorMsg: Plz_select_question_type });
    } else if (myQuestions[length - 1].type == 'rating_scale') {
      if (
        !myQuestions[length - 1].title ||
        myQuestions[length - 1].title.length < 1
      ) {
        this.setState({ errorMsg: Title_cant_be_empty });
      } else if (
        !myQuestions[length - 1].question ||
        myQuestions[length - 1].question.length < 1
      ) {
  ...
  editQuestion = (data, _id) => {
    ...
    this.setState({
      editQuestions: deep,
      editQues: true,
      editopenOpti: data?.type === 'classic' ? true : false,
    });
  };
  ...
  SelectedValue = (value) => {
    var selected =
      options?.length > 0 && options.filter((e) => e.value === value);
  ...
  render() {
    ...
    let {
      ...
      Add_a_Choice,
      Questionnairetitle,
      Make_it_multiple_questionnaire,
      Choose_questionnaire_type,
      NewQuestion,
                ...
                <Grid item xs={12} md={10}>
                  <Grid className="topLeftSpc">
                    <Grid container direction="row">
                      <Grid item xs={6} md={6}>
                        {/* Back common button */}
                        <Grid className="extSetting"></Grid>
