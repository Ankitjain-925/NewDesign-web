...
export const getSpecialty = (current) => {
  ...
  export const getSpecialtyData = (id, current) => {
    if (id) {
      ...
      if (id === 'general') {
        var filterData = current.state.AllServices?.length > 0 && current.state.AllServices.filter((data) => !data.speciality_id)
      }
      else {
        var filterData = current.state.AllServices?.length > 0 && current.state.AllServices.filter((data) => (data?.speciality_id && data?.speciality_id.includes(id)))
      }
    }
    else {
