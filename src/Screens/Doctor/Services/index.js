...
class Index extends Component {
          ...
          axios
            ...
            .then((responce) => {
              if (responce.data.hassuccessed) {
                axios
                  .post(
                    sitedata.data.path + "/UserProfile/AskPatient/" + user_id,
                    {
    ...
    if (
      ...
      if (this.validateEmail(userDetails.email)) {
        if (
          ...
          userDetails.password.match(letter) &&
          userDetails.password.match(number) &&
    ...
    axios
      ...
      .then((response) => {
        ...
        if (response.data.hassuccessed) {
          ...
          this.setState({
            patDeleteErr:
              "Something happened Wrong, Patient delete unsuccessfull",
          });
          setTimeout(() => {
            this.setState({ patDeleteErr: null });
          }, 3000);
        }
      })
      .catch((error) => {
  ...
  onChangePage = (pageNumber) => {
    ...
    if (searchWord && searchWord !== "") {
      let searchdta = this.state.AllPres.filter(
        (e) =>
  ...
  render() {
    ...
    let {
      ...
      view_data,
      private_doc_rest,
      ask_patient_to_become_a_private_doctor,
      Ask,
      email_or_id,
      enter_patient_email_id,
      new_patient,
      add_new_patient,
      first,
      name,
                          ...
                          <Grid className="infoCourse">
                            <Grid className="shPatientProfile">
                              ...
                              <Grid className="userDetail">
                                <Grid className="userDetailLft">
                                  <img src={profileDetail.new_image} />
                                </Grid>
                                <Grid className="userDetailRght">
                                  ...
                                  <Grid>
                                    <p>
                                      {`${moment(profileDetail.birthday).format(
                                        "DD/MM/YYYY"
                                      )}`}
                            ...
                            <Grid className="userAdresInfo">
                              <Grid className="userAdres">
                                ...
                                <p>
                                  {profileDetail.address} <br />
                                  {profileDetail.city} <br />
                                  {profileDetail.country
                                    ? profileDetail.country.label
                                    : ""}
                                </p>
                              </Grid>
                              <Grid className="userAdres">
                              ...
                              <Grid className="userAdres">
                                ...
                                <p>
                                  {profileDetail.language &&
                                    profileDetail.language.map((data, inde) =>
                                      inde !== 0 ? ", " + data : data
                                    )}
                                </p>
                              </Grid>
                              <Grid className="insureMe">
                                ...
                                {profileDetail.insurance &&
                                  profileDetail.insurance.map((data, index) => (
                                    <Grid container direction="row">
                                      <Grid item xs={4} md={4}>
                                        <p>
                                          {data
                                ...
                                >
                                  <Grid item xs={12} md={12}>
                                    <input
                                      type="submit"
                                      onClick={() => {
                                        this.GotoJournal(profileDetail);
                                      }}
                                      value={openjournal}
                                    />
                                  </Grid>
                          ...
                          <Grid className="dataBoxUpr">
                            <Grid className="dataBoxInput entrpatId">
                              ...
                              {this.state.sentmessages && (
                                <div className="success_message">{succ1}</div>
                              )}
                              <Grid>
                                <Grid>
                                  <label>{email_or_id}</label>
                                </Grid>
                                <Grid>
                                  <input
                                    type="text"
                                    placeholder={enter_patient_email_id}
                                    onChange={(event) => {
                                      this.setState({
                                        AskPatient: event.target.value,
                                        errorSentMsg: "",
                                      });
                                    }}
                                  />
