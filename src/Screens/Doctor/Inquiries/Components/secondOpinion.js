...
class Index extends Component {
  ...
  UpdatetheStatus = (status, id) => {
    ...
    axios
      .put(
        sitedata.data.path + "/UserProfile/GetSecondOpinion/" + id,
        {
          status: status,
          doctor_name:
  ...
  render() {
    ...
    let {
      ...
      how_would_patient_like_to_rcv_secon_openion,
      patient_profession,
      attachments,
      attached_doc,
    } = translate;
                  ...
                  <Grid className="stndrdQues">
                    ...
                    <Grid className="splestQues">
                      ...
                      <Grid>
                        <h3>
                          {opinionData &&
                            opinionData.patient_info &&
                            opinionData.patient_info.first_name &&
                            opinionData.patient_info.first_name}{" "}
                          {opinionData &&
                            opinionData.patient_info &&
                            opinionData.patient_info.last_name &&
                            opinionData.patient_info.last_name}
                        </h3>
                      </Grid>
                    </Grid>
                    <Grid className="recevPrescp">
                      ...
                      <Grid className="recevPrescpChk">
                        {opinionData.online_offline === "online"
                          ? online
                          : home_add_mailbox}
                      </Grid>
                    </Grid>
                    <Grid className="yrProfes">
                      <Grid>
                        <label>{patient_profession}</label>
                      </Grid>
                    ...
                    <Grid className="attchForms attchImg">
                      ...
                      <label className="attached_file">
                        {attached_doc} -
                        <ul>
                        {opinionData && opinionData.documents && opinionData.documents.map((datas)=>(
                        <li>
                          <a onClick={()=>{GetUrlImage(datas.filename)}}>
                            {datas.filename &&
                              datas.filename
                        ...
                        {opinionData && opinionData.attachfile && (
                          <a onClick={()=>{GetUrlImage(opinionData.attachfile)}}>
                            {opinionData.attachfile.filename &&
                              opinionData.attachfile.filename
                                .split("Trackrecord/")[1]
                                .split("&bucket=")[0]}
                          </a>
                      ...
                      {opinionData.status !== "accept" && !$imagePreview && (
                        <Dropzone
                          onDrop={(e) =>
                            this.UploadFile(
                              e,
                              opinionData.patient_profile_id,
                              opinionData.patient_info.bucket,
                              opinionData._id
                            )
                          }
                        >
                    ...
                    <Grid className="infoShwHidIner">
                      <Grid className="infoShwSave">
                        {opinionData.status !== "accept" &&
                          opinionData.status !== "decline" && (
