...
class Index extends Component {
  ...
  getUserData() {
    ...
    axios
      ...
      .then((response) => {
        ...
        if (privateAppointments) {
          if (privateAppointments.holidays) {
            this.setState({
              holidayAppointment: {
                holidays_start:
                  privateAppointments.holidays_start !== ""
                    ? privateAppointments.holidays_start
                    : new Date(),
                holidays_end:
                  privateAppointments.holidays_end !== ""
                    ? privateAppointments.holidays_end
                    : new Date(),
          ...
          this.setState({
            UpDataDetails: privateAppointments,
            StandardSetting: privateAppointments,
            CustomName: privateAppointments,
          });
        } else {
          privateAppointments = {};
          keysArray.map((key) => {
        ...
        if (daysForPractices) {
          if (daysForPractices.holidays) {
            this.setState({
              holidayAppointment: {
                holidays_start:
                  daysForPractices.holidays_start !== ""
                    ? daysForPractices.holidays_start
                    : new Date(),
                holidays_end:
                  daysForPractices.holidays_end !== ""
                    ? daysForPractices.holidays_end
                    : new Date(),
          ...
          this.setState({
            DaysforPractices: daysForPractices,
        ...
        if (onlineAppointment) {
          if (onlineAppointment.holidays) {
            this.setState({
              holidayAppointment: {
                holidays_start:
                  onlineAppointment.holidays_start !== ""
                    ? onlineAppointment.holidays_start
                    : new Date(),
                holidays_end:
                  onlineAppointment.holidays_end !== ""
                    ? onlineAppointment.holidays_end
                    : new Date(),
          ...
          this.setState({
            ...
            OnlineSetting: onlineAppointment,
          });
        } else {
          onlineAppointment = {};
          keysArray.map((key) => {
        ...
        let firstServiceData = this.state.paid_services.find(
          (ele) => ele.description === "videochat"
        );
        if (firstServiceData)
          this.setState({ firstServiceData: firstServiceData });

        let sencondSeviceData = this.state.paid_services.find(
          (ele) => ele.description === "prescription"
        );
        if (sencondSeviceData)
          this.setState({ sencondSeviceData: sencondSeviceData });

        let thirdServiceData = this.state.paid_services.find(
          (ele) => ele.description === "appointment"
        );
        if (thirdServiceData)
          this.setState({ thirdServiceData: thirdServiceData });
      })
      .catch((error) => {
        this.setState({ loaderImage: false });
      });
  ...
  saveAllData = () => {
    ...
    if (
      ...
      (!UpDataDetails.duration_of_timeslots ||
        UpDataDetails.duration_of_timeslots === 0)
    ) {
      this.setState({ appoinmentError: true });
    } else if (
      weoffer &&
      weoffer.Offer_practice_appointment &&
      (!DaysforPractices.duration_of_timeslots ||
        DaysforPractices.duration_of_timeslots === 0)
    ) {
      this.setState({ appoinmentError: true });
    } else if (
      weoffer &&
      weoffer.Offre_online_appointments &&
      (!onlineAppointments.duration_of_timeslots ||
      ...
      if (holidayAppointment["holidays"]) {
        dataSave["days_for_practices"]["holidays"] =
          holidayAppointment["holidays"];
        dataSave["days_for_practices"]["holidays_start"] = moment(
          holidayAppointment["holidays_start"]
        ).format("YYYY-MM-DD");
        dataSave["days_for_practices"]["holidays_end"] = moment(
          holidayAppointment["holidays_end"]
        ).format("YYYY-MM-DD");
        dataSave["online_appointment"]["holidays"] =
          holidayAppointment["holidays"];
        dataSave["online_appointment"]["holidays_start"] = moment(
          holidayAppointment["holidays_start"]
        ).format("YYYY-MM-DD");
        dataSave["online_appointment"]["holidays_end"] = moment(
          holidayAppointment["holidays_end"]
        ).format("YYYY-MM-DD");
        dataSave["private_appointments"]["holidays"] =
          holidayAppointment["holidays"];
        dataSave["private_appointments"]["holidays_start"] = moment(
          holidayAppointment["holidays_start"]
        ).format("YYYY-MM-DD");
        dataSave["private_appointments"]["holidays_end"] = moment(
          holidayAppointment["holidays_end"]
        ).format("YYYY-MM-DD");
      } else {
        dataSave["days_for_practices"]["holidays"] =
  ...
  saveOnlineAppointments() {
    if (
      this.state.onlineAppointments.duration_of_timeslots &&
      this.state.onlineAppointments.duration_of_timeslots !== 0
    ) {
      ...
      axios
        .put(
          sitedata.data.path + "/UserProfile/onlineAppointments/" + doctor_id,
          {
            ...
            duration_of_timeslots: this.state.onlineAppointments
              .duration_of_timeslots,
            holidays_start: holidays_start,
            holidays_end: holidays_end,
  ...
  render() {
    ...
    let {
      ...
      copy_time_to_all,
      chat_vdocall,
      prescription_and_sick_cert,
      online_prescription,
      select_appointment_set_schedule,
      holiday_period_btw,
      appointment_system,
      issue_prescription_and_sickCert_online,
      onine_sick_cert,
      appointments,
      offer_practice_online_appointment,
      set_timeslot_duration,
      break_time,
              ...
              <Grid item xs={12} md={6}>
                <Grid className="prescSickUpr">
                  <Grid className="prescSick">
                    ...
                    <Grid>
                      <Grid>
                        <label>{prescription_and_sick_cert}</label>
                      </Grid>
                      ...
                      <FormControlLabel
                        control={
                          <Checkbox
                            ...
                            color="#00ABAF"
                            checked={
                              weoffer && weoffer.Offer_online_sick_certificates
                                ? true
                                : false
                            }
                            onChange={() =>
                              this.handleweoffer(
                                "Offer_online_sick_certificates"
                              )
                            }
                          />
                        }
                      ...
                      <FormControlLabel
                        control={
                          <Checkbox
                            ...
                            color="#00ABAF"
                            checked={
                              weoffer && weoffer.Offer_online_prescription
                                ? true
                                : false
                            }
                            onChange={() =>
                              this.handleweoffer("Offer_online_prescription")
                            }
                          />
                        }
                ...
                <Grid className="enableTogle">
                  <label>
                    <Toggle
                      ...
                      checked={
                        sencondSeviceData &&
                        sencondSeviceData.description == "prescription"
                      }
                      onClick={(e) => {
                        this.handlepaidService(e, "sencondSeviceData");
                      }}
                    />
                  </label>
                </Grid>
                ...
                <Grid className="enableTogle">
                  <Grid>
                    <label>
                      <Toggle
                        icons={false}
                        checked={
                          holidayAppointment && holidayAppointment.holidays
                        }
                        onClick={(e) => {
                          this.handleholiday("holidayAppointment");
                        }}
                      />
                  ...
                  {holidayAppointment && holidayAppointment.holidays && (
                    <Grid className="enblDate">
                      <RangePicker
                        onChange={(date) =>
                          this.handleholidayDate("holidayAppointment", date)
                        }
                        defaultValue={[
                          moment(
                            holidayAppointment.holidays_start,
                  ...
                  <FormControlLabel
                    control={
                      <Checkbox
                        ...
                        color="#00ABAF"
                        checked={
                          weoffer && weoffer.Offre_online_appointments
                            ? true
                            : false
                        }
                        onChange={() =>
                          this.handleweoffer("Offre_online_appointments")
                        }
                      />
                    }
                    label={vdo_call}
                ...
                <Grid className="enableTogle">
                  <label>
                    <Toggle
                      ...
                      checked={
                        thirdServiceData &&
                        thirdServiceData.description == "appointment"
                      }
                      onClick={(e) => {
                        this.handlepaidService(e, "thirdServiceData");
                      }}
                    />
                  </label>
                </Grid>
              ...
              <Grid container direction="row">
                <Grid item xs={12} md={6}>
                  <Grid className="wrkHour">
                    <Grid>
                      <label>{set_online_working_hour}</label>
                    </Grid>
                    <Grid>
                      <a
                        className={
                          onlineAppointments.monday_end !== "" && "seleted-days"
                        }
                        onClick={() =>
                          this.selectWeek("onlineAppointments", "monday")
                        }
                      >
                        M
                      </a>
                      <a
                        className={
                          onlineAppointments.tuesday_end !== "" &&
                          "seleted-days"
                        }
                        onClick={() =>
                          this.selectWeek("onlineAppointments", "tuesday")
                        }
                      >
                        T
                      </a>
                      <a
                        ...
                        onClick={() =>
                          this.selectWeek("onlineAppointments", "wednesday")
                        }
                      >
                        {" "}
                        W
                      ...
                      <a
                        ...
                        onClick={() =>
                          this.selectWeek("onlineAppointments", "thursday")
                        }
                      >
                        T
                      </a>
                      <a
                        className={
                          onlineAppointments.friday_end !== "" && "seleted-days"
                        }
                        onClick={() =>
                          this.selectWeek("onlineAppointments", "friday")
                        }
                      >
                        F
                      </a>
                      <a
                        ...
                        onClick={() =>
                          this.selectWeek("onlineAppointments", "saturday")
                        }
                      >
                        S
                      </a>
                      <a
                        className={
                          onlineAppointments.sunday_end !== "" && "seleted-days"
                        }
                        onClick={() =>
                          this.selectWeek("onlineAppointments", "sunday")
                        }
                      >
                        S
                      </a>
                        ...
                        <Grid className="daySchedule">
                          ...
                          <Grid>
                            ...
                            <TimeFormat
                              ...
                              value={
                                onlineAppointments.monday_end
                                  ? this.getTime(onlineAppointments.monday_end)
                                  : new Date()
                              }
                              time_format={
                                this.props.settings &&
                          ...
                          <Grid>
                            <p
                              onClick={() =>
                                this.copytoall("onlineAppointments", "monday")
                              }
                            >
                              <img
                        ...
                        <Grid className="daySchedule">
                          ...
                          <Grid>
                            ...
                            <TimeFormat
                              ...
                              value={
                                onlineAppointments.tuesday_end
                                  ? this.getTime(onlineAppointments.tuesday_end)
                                  : new Date()
                              }
                              time_format={
                                this.props.settings &&
                        ...
                        <Grid className="daySchedule">
                          ...
                          <Grid>
                            ...
                            <TimeFormat
                              ...
                              value={
                                onlineAppointments.friday_end
                                  ? this.getTime(onlineAppointments.friday_end)
                                  : new Date()
                              }
                              time_format={
                                this.props.settings &&
                        ...
                        <Grid className="daySchedule">
                          ...
                          <Grid>
                            ...
                            <TimeFormat
                              ...
                              value={
                                onlineAppointments.sunday_end
                                  ? this.getTime(onlineAppointments.sunday_end)
                                  : new Date()
                              }
                              time_format={
                                this.props.settings &&
                    ...
                    <Grid className="setSchedule">
                      ...
                      <Grid className="nameSchedule">
                        <input
                          ...
                          value={onlineAppointments.duration_of_timeslots}
                          onChange={(e) =>
                            this.changeDuration(e, "onlineAppointments")
                          }
                        />{" "}
                        {minutes}
                      </Grid>
                  ...
                  <Grid className="apontBook">
                    ...
                    <Grid>
                      <p>
                        ...
                        <input
                          ...
                          value={onlineAppointments.appointment_days}
                        />{" "}
                        {before_day_of_appointment}
                      </p>
                    </Grid>
                    <Grid>
                      <p>
                        ...
                        <input
                          type="text"
                          value={onlineAppointments.appointment_hours}
                          onChange={(e) =>
                            this.onChangebook(
                              e,
                              "appointment_hours",
                  ...
                  <FormControlLabel
                    control={
                      <Checkbox
                        ...
                        color="#00ABAF"
                        checked={
                          weoffer && weoffer.Offer_practice_appointment
                            ? true
                            : false
                        }
                        onChange={() =>
                          this.handleweoffer("Offer_practice_appointment")
                        }
                      />
                    }
                ...
                <Grid item xs={12} md={6}>
                  <Grid className="wrkHour">
                    ...
                    <Grid>
                      <a
                        className={
                          DaysforPractices.monday_end !== "" && "seleted-days"
                        }
                        onClick={() =>
                          this.selectWeek("DaysforPractices", "monday")
                        }
                      >
                        M
                      </a>
                      <a
                        className={
                          DaysforPractices.tuesday_end !== "" && "seleted-days"
                        }
                        onClick={() =>
                          this.selectWeek("DaysforPractices", "tuesday")
                        }
                      >
                        T
                      </a>
                      <a
                        ...
                        onClick={() =>
                          this.selectWeek("DaysforPractices", "wednesday")
                        }
                      >
                        {" "}
                        W
                      ...
                      <a
                        className={
                          DaysforPractices.thursday_end !== "" && "seleted-days"
                        }
                        onClick={() =>
                          this.selectWeek("DaysforPractices", "thursday")
                        }
                      >
                        T
                      </a>
                      <a
                        className={
                          DaysforPractices.friday_end !== "" && "seleted-days"
                        }
                        onClick={() =>
                          this.selectWeek("DaysforPractices", "friday")
                        }
                      >
                        F
                      </a>
                      <a
                        className={
                          DaysforPractices.saturday_end !== "" && "seleted-days"
                        }
                        onClick={() =>
                          this.selectWeek("DaysforPractices", "saturday")
                        }
                      >
                        S
                      </a>
                      <a
                        className={
                          DaysforPractices.sunday_end !== "" && "seleted-days"
                        }
                        onClick={() =>
                          this.selectWeek("DaysforPractices", "sunday")
                        }
                      >
                        S
                      </a>
                        ...
                        <Grid className="daySchedule">
                          ...
                          <Grid>
                            <TimeFormat
                              name="time"
                              value={
                                DaysforPractices.monday_start
                                  ? this.getTime(DaysforPractices.monday_start)
                                  : new Date()
                              }
                              time_format={
                                this.props.settings &&
                            ...
                            <TimeFormat
                              ...
                              value={
                                DaysforPractices.monday_end
                                  ? this.getTime(DaysforPractices.monday_end)
                                  : new Date()
                              }
                              time_format={
                                this.props.settings &&
                          ...
                          <Grid>
                            <p
                              onClick={() =>
                                this.copytoall("DaysforPractices", "monday")
                              }
                            >
                              <img
                                src={require("../../../../assets/images/docscopy.svg")}
                        ...
                        <Grid className="daySchedule">
                          ...
                          <Grid>
                            <TimeFormat
                              name="time"
                              value={
                                DaysforPractices.tuesday_start
                                  ? this.getTime(DaysforPractices.tuesday_start)
                                  : new Date()
                              }
                              time_format={
                                this.props.settings &&
                            ...
                            <TimeFormat
                              ...
                              value={
                                DaysforPractices.tuesday_end
                                  ? this.getTime(DaysforPractices.tuesday_end)
                                  : new Date()
                              }
                              time_format={
                                this.props.settings &&
                          ...
                          {DaysforPractices.monday_end == "" && (
                            <Grid>
                              <p
                                onClick={() =>
                                  this.copytoall("DaysforPractices", "tuesday")
                                }
                              >
                                <img
                                  src={require("../../../../assets/images/docscopy.svg")}
                        ...
                        <Grid className="daySchedule">
                          ...
                          <Grid>
                            ...
                            <TimeFormat
                              ...
                              value={
                                DaysforPractices.wednesday_end
                                  ? this.getTime(DaysforPractices.wednesday_end)
                                  : new Date()
                              }
                              time_format={
                                this.props.settings &&
                        ...
                        <Grid className="daySchedule">
                          ...
                          <Grid>
                            ...
                            <TimeFormat
                              ...
                              value={
                                DaysforPractices.thursday_end
                                  ? this.getTime(DaysforPractices.thursday_end)
                                  : new Date()
                              }
                              time_format={
                                this.props.settings &&
                        ...
                        <Grid className="daySchedule">
                          ...
                          <Grid>
                            <TimeFormat
                              name="time"
                              value={
                                DaysforPractices.friday_start
                                  ? this.getTime(DaysforPractices.friday_start)
                                  : new Date()
                              }
                              time_format={
                                this.props.settings &&
                            ...
                            <TimeFormat
                              ...
                              value={
                                DaysforPractices.friday_end
                                  ? this.getTime(DaysforPractices.friday_end)
                                  : new Date()
                              }
                              time_format={
                                this.props.settings &&
                          ...
                          {DaysforPractices.monday_end == "" &&
                            ...
                            DaysforPractices.thursday_end == "" && (
                              <Grid>
                                <p
                                  onClick={() =>
                                    this.copytoall("DaysforPractices", "friday")
                                  }
                                >
                                  <img
                                    src={require("../../../../assets/images/docscopy.svg")}
                        ...
                        <Grid className="daySchedule">
                          ...
                          <Grid>
                            ...
                            <TimeFormat
                              ...
                              value={
                                DaysforPractices.saturday_end
                                  ? this.getTime(DaysforPractices.saturday_end)
                                  : new Date()
                              }
                              time_format={
                                this.props.settings &&
                        ...
                        <Grid className="daySchedule">
                          ...
                          <Grid>
                            <TimeFormat
                              name="time"
                              value={
                                DaysforPractices.sunday_start
                                  ? this.getTime(DaysforPractices.sunday_start)
                                  : new Date()
                              }
                              time_format={
                                this.props.settings &&
                            ...
                            <TimeFormat
                              ...
                              value={
                                DaysforPractices.sunday_end
                                  ? this.getTime(DaysforPractices.sunday_end)
                                  : new Date()
                              }
                              time_format={
                                this.props.settings &&
                          ...
                          {DaysforPractices.monday_end == "" &&
                            ...
                            DaysforPractices.saturday_end == "" && (
                              <Grid>
                                <p
                                  onClick={() =>
                                    this.copytoall("DaysforPractices", "sunday")
                                  }
                                >
                                  <img
                                    src={require("../../../../assets/images/docscopy.svg")}
                    ...
                    <Grid className="setSchedule">
                      ...
                      <Grid className="nameSchedule">
                        <input
                          type="text"
                          name="duration_of_timeslots"
                          value={DaysforPractices.duration_of_timeslots}
                          onChange={(e) =>
                            this.changeDuration(e, "DaysforPractices")
                          }
                        />{" "}
                        {minutes}
                      </Grid>
                  ...
                  <Grid className="apontBook">
                    ...
                    <Grid>
                      <p>
                        ...
                        <input
                          ...
                          value={DaysforPractices.appointment_days}
                        />
                        {before_day_of_appointment}
                      </p>
                    </Grid>
                    <Grid>
                      <p>
                        ...
                        <input
                          type="text"
                          value={DaysforPractices.appointment_hours}
                          onChange={(e) =>
                            this.onChangebook(
                              e,
                              "appointment_hours",
