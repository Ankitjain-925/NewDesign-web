...
class Index extends Component {
  ...
  deleteProf = (index) => {
    var ProfAy =
      this.state.professional_data?.length > 0 &&
      this.state.professional_data.filter((data, index1) => index1 !== index);
      ...
      .then((response) => {
        if (response.data.hassuccessed) {
          ...
          for (let i = 0; i < this.state.allPatData.length; i++) {
            ...
            PatientList1.push({
              profile_id: this.state.allPatData[i].patient?.profile_id,
              value: this.state.allPatData[i].patient?.patient_id,
              name: name,
            });
          }
                    ...
                    <Grid container direction="row">
                      ...
                      <Grid item xs={12} md={6}>
                        <Grid className="addTaskBtn">
                          <Button onClick={this.handleOpenTask}>
                            + Add Task
                          </Button>
                        </Grid>
                      </Grid>
