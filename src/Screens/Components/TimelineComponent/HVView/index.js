...
class Index extends Component {
    ...
    render() {
        ...
        let { hosp_visit, visibility, Download, Delete, visible, hide, details, img_files,  title, show, first_day_visit, always, edit, date_of_death, date_of_dieses_patient, dob, day_doc_visit, gender_of_relatives, relation_of_relative, Change, speciality, hosp_id, hosp_name, doc_id, traveled, slct_ICD_serch_code, when, to, allergy, enter_code_serch_by_keyword, dignose, of, until, archive, rr_systolic, attachments, time_measure, date_measure,
                        ...
                        <Grid container direction="row" className="addSpc">
                            <Grid item xs={12} md={6}>
                                <Grid className="blodPrsurImg purpleSpan">
                                    <a className="diryNote"><img src={require('../../../../assets/images/hospital-visit.svg')} alt="" title="" />
                                        <span>{hosp_visit}</span>
                                    </a>
                                </Grid>
                            </Grid>
                            <Grid item xs={12} md={6}>
...
<li><DownloadFullTrack TrackRecord={this.state.TrackRecord}/></li>
                        ...
                        <Grid className="bp_hg addSpc">
                            <label>{item.hospital_name && item.hospital_name} <span></span></label>
                            {/* <p>Normal</p> */}
                        </Grid>
                                ...
                                <Grid className="detailCntnt">
                                    <Grid container direction="row">
                                        <Grid item xs={12} md={6} className="bloodPreBy">
                                            <Grid container direction="row">
                                                <Grid item xs={5} md={5}><label>{hosp_name}</label></Grid>
                                                <Grid item xs={7} md={7}><span>{item.hospital_name && item.hospital_name}</span></Grid>
                                                <Grid className="clear"></Grid>
                                            </Grid>
                                        </Grid>
                                        <Grid item xs={12} md={6} className="bloodPreBy">
                                            <Grid container direction="row">
                                                <Grid item xs={5} md={5}><label>{hosp_id}</label></Grid>
                                                <Grid item xs={7} md={7}><span>{item.hospital_id && item.hospital_id}</span></Grid>
                                                <Grid className="clear"></Grid>
                                            </Grid>
                                        </Grid>
                                        <Grid item xs={12} md={6} className="bloodPreBy">
                                            <Grid container direction="row">
                                                <Grid item xs={5} md={5}><label>{doc_id}</label></Grid>
                                                <Grid item xs={7} md={7}><span>{item.doctor_id && item.doctor_id}</span></Grid>
                                                <Grid className="clear"></Grid>
                                            </Grid>
                                        </Grid>
                                        <Grid item xs={12} md={6} className="bloodPreBy">
                                            <Grid container direction="row">
                                                ...
                                                <Grid item xs={7} md={7}><span>{item.specialty && GetShowLabel1(this.props.AllSpecialty, item.specialty.value, this.props.stateLanguageType, true,'specialty')}</span></Grid>
                                                <Grid className="clear"></Grid>
                                            </Grid>
                                        </Grid>
                                        <Grid item xs={12} md={6} className="bloodPreBy">
                                            <Grid container direction="row">
                                                <Grid item xs={5} md={5}><label>{first_visit_day}</label></Grid>
                                                <Grid item xs={7} md={7}><span>{item.first_visit_date && getDate(item.first_visit_date, this.state.date_format)} </span></Grid>
                                                <Grid className="clear"></Grid>
                                            </Grid>
                                        </Grid>
                                        ...
                                        <Grid item xs={12} md={6} className="bloodPreBy">
                                            <Grid container direction="row">
                                                <Grid item xs={5} md={5}><label>{last_visit_day}</label></Grid>
                                                <Grid item xs={7} md={7}><span>{item.last_visit_date && getDate(item.last_visit_date, this.state.date_format)} </span></Grid>
                                                <Grid className="clear"></Grid>
                                            </Grid>
                                        </Grid>
                                        <Grid className="clear"></Grid>
