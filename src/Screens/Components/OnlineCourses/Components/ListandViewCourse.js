...
class Index extends Component {
  ...
  ChangeIndex = (index) => {
    this.setState(
      {
        CurrentIndex: index,
        CurrentAttach: this.state.Course.attachment[index],
      },
      () => {
      ...
      <Grid className="courseCntent">
        <Grid container direction="row">
          <Grid item xs={12} md={3} className="myCoursesOnline">
            ...
            <Grid className="whtDiabts">
              <h3>{this.state.Course.courseTitle}</h3>
            </Grid>
            <Grid className="videoTitle">
              <ul>
                {this.state.Course &&
                  this.state.Course.attachment &&
                  this.state.Course.attachment.length > 0 &&
                  this.state.Course.attachment.map((items, index) => (
                    <li
                      onClick={() => {
          ...
          <Grid item xs={12} md={8}>
            <Grid className="lessonTitle">
              ...
              <Grid className="lessonTitleVdo">
                ...
                <Iframeview
                  new_image={this.state.CurrentAttach.new_image}
                  type={this.state.CurrentAttach.type}
                  comesFrom="LMS"
                />
              </Grid>
              ...
              <Grid container direction="row" alignItems="center">
                ...
                <Grid item xs={6} md={6} className="nxtLesson">
                  {this.state.CurrentIndex < this.state.NextButton && (
                    <a
                      onClick={() =>
                        this.ChangeIndex(this.state.CurrentIndex + 1)
                      }
