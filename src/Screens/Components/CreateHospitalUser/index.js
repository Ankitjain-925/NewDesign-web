...
class Index extends Component {
  ...
  getAllinst = () => {
    ...
    axios
      ...
      .then((response) => {
        this.setState({
          getHintInstitute: response.data.data,
          getHintInstitute1: response.data.data,
        });
      })
      .catch((error) => {});
  };
  ...
  handleCloseCreate = () => {
    this.setState({ CreateUsers: {}, regisError0: '', error_msg: '' });
  ...
  componentDidMount = () => {
    if (this.props.openBy === 'patient') {
      this.setState({
        specialistOption: {
          label: this.props.stateLanguageType === 'en' ? 'Patient' : 'Geduldig',
          value: 'patient',
        },
        CreateUsers: { type: 'patient' },
      });
    }
    if (this.props.openBy === 'doctor') {
      this.setState({
        specialistOption: {
          label: this.props.stateLanguageType === 'en' ? 'Doctor' : 'Ärztin',
          value: 'doctor',
        },
        CreateUsers: { type: 'doctor' },
      });
    }
    if (this.props.openBy === 'nurse') {
      this.setState({
        specialistOption: {
          label:
            this.props.stateLanguageType === 'en'
              ? 'Nurse'
              : 'Krankenschwester',
          value: 'nurse',
        },
        CreateUsers: { type: 'nurse' },
      });
    }
  ...
  saveUserData = () => {
    ...
    if (
      this.state.CreateUsers.first_name &&
      this.state.CreateUsers.last_name &&
      this.state.CreateUsers.first_name !== '' &&
      this.state.CreateUsers.last_name !== ''
    ) {
      if (this.validateEmail(this.state.CreateUsers.email)) {
        if (
          ...
          this.state.CreateUsers.password &&
          this.state.CreateUsers.password.match(letter) &&
          this.state.CreateUsers.password.match(number) &&
          this.state.CreateUsers.password.match(specialchar)
        ) {
          if (
            this.state.CreateUsers &&
            this.state.CreateUsers.password &&
            this.state.CreateUsers.password === this.state.confirmPass
          ) {
            if (
              this.state.CreateUsers.mobile &&
              this.state.CreateUsers.mobile !== ''
            ) {
              if (
                this.state.CreateUsers.type &&
                this.state.CreateUsers.type !== ''
              ) {
                var datas =
                  this.state.getHintInstitute &&
                  this.state.getHintInstitute.length > 0 &&
                  this.state.getHintInstitute.filter(
                    (value, key) =>
                      value.created_by ===
                ...
                axios
                  .post(
                    sitedata.data.path + '/admin/Addadminuser',
                    {
                  ...
                  .then((response) => {
                    if (response.data.hassuccessed) {
                      ...
                      setTimeout(() => {
                        ...
                        this.props.handleCloseCreate();
                        if (UserType === 'patient') {
                          this.props.history.push('/h-patients');
                        } else if (UserType === 'doctor') {
                          this.props.history.push('/h-doctors');
                        } else if (UserType === 'nurse') {
                          this.props.history.push('/h-nurses');
                        } else if (UserType === 'adminstaff') {
                          this.props.history.push('/h-staff');
                        }
                      }, 2000);
                    } else {
                      this.setState({
            ...
            this.setState({
              regisError0: 'Password and confirm password is not matching',
            });
          }
        } else {
  ...
  render() {
    ...
    return (
      <div>
        ...
        <Modal open={this.state.addCreate} onClose={this.handleCloseCreate}>
          <Grid
            className={
              this.props.settings &&
                ...
                <Grid className="emlCreatSub">
                  ...
                  <Grid>
                    <input
                      type="text"
                      name="email"
                      placeholder={recEmp_Email}
                      value={this.state.CreateUsers.email}
                      onChange={this.CreateUserState}
                    />
                  </Grid>
                </Grid>
                <Grid className="emlCreatSub">
                  <Grid>
                    <label>{current_pass}</label>
                  </Grid>
                  <Grid className="registerPass">
                    <input
                      type={this.state.hidden ? 'password' : 'text'}
                      name="password"
                      value={this.state.CreateUsers.password}
                      onChange={this.CreateUserState}
                    />
                    {this.state.hidden && (
                    ...
                    <div className="passInstIner">
                      ...
                      <ul>
                        <li>
                          {this.state.CreateUsers &&
                            this.state.CreateUsers.password &&
                            this.state.CreateUsers.password.length > 8 && (
                              <a>
                                <img
                        ...
                        <li>
                          {this.state.CreateUsers &&
                            this.state.CreateUsers.password &&
                            !this.state.CreateUsers.password.match(
                              specialchar
                            ) && (
                              <a>
                                <img
                ...
                <Grid className="emlCreatSub">
                  ...
                  <Grid>
                    <input
                      type="text"
                      name="first_name"
                      value={this.state.CreateUsers.first_name}
                      onChange={this.CreateUserState}
                    />
                  </Grid>
                </Grid>
                ...
                <Grid className="emlCreatSub">
                  ...
                  <Grid>
                    <input
                      type="text"
                      name="last_name"
                      value={this.state.CreateUsers.last_name}
                      onChange={this.CreateUserState}
                    />
                  </Grid>
                </Grid>
                ...
                <Grid className="emlCreatSub">
                  ...
                  <Grid className="showflags">
                    ...
                    <input
                      ...
                      className="mobileReg"
                      name="mobile"
                      value={this.state.CreateUsers.mobile}
                      onChange={this.CreateUserState}
                    />
                  </Grid>
                </Grid>
